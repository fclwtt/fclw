...
$headTemplet = "<li><a id='sunitems~cc~' href='javascript:void(0);' class='nav-top-item'>~channelname~</a><ul>\r\n";
    ...
    foreach($dtp->CTags as $i=>$ctag)
    {
        if($ctag->GetName()=='top' && ($ctag->GetAtt('rank')=='' || TestPurview($ctag->GetAtt('rank')) ))
        {
            ...
            foreach($dtp2->CTags as $j=>$ctag2)
            {
                ...
                {
                    $link = "<a class='nav-bottom-item' href='".$ctag2->GetAtt('link')."' target='".$ctag2->GetAtt('target')."'>".$ctag2->GetAtt('name')."</a>";
                    if($ischannel=='1')
                    {
